// Verify the player capsule cannot walk south off the wrap walkway. At each
// x along the walkway, place the capsule at z=0.65 (mid-walkway) and ram it
// south with a strong velocity for 1.5s. Confirm settled z stays <= 1.3
// (i.e. on or just inside the walkway), and footY stays at 4 (no fall).
...
const proc = spawn(CHROME, ['--remote-debugging-port=' + PORT, '--window-position=2000,2000', '--user-data-dir=/tmp/chrome-diag-south', '--no-first-run', URL], { stdio: ['ignore', 'pipe', 'pipe'] });
let target = null;
...
const results = [];
for (let x = -18; x <= 12; x += 2) {
  const r = await ramSouth(x, 0.6);
  results.push({ startX: x, settledX: +r.x.toFixed(2), settledZ: +r.z.toFixed(2), footY: +r.footY.toFixed(2) });
}
// Also ram east at east terminus (x=12) and west at west terminus (x=-18):
const eastEdge = await evalExpr(`(() => {
  const G = window.__G, Player = G.Player, Colliders = G.Colliders, THREE = G.THREE, R = G.resolveSphereAABB;
  Player.pos.set(11, 4.0 + Player._eyeH, 0.6); Player.vel.set(0,0,0);
  const dt = 1/60;
  for (let f = 0; f < 120; f++) {
    Player.vel.x = 8.0; Player.vel.y -= 22 * dt;
    Player.pos.addScaledVector(Player.vel, dt);
    const r = Player.radius, bH = Player._bodyH, list = Colliders.list;
    const _b = new THREE.Vector3(), _t = new THREE.Vector3();
    for (let pass = 0; pass < 4; pass++) {
...
console.log(JSON.stringify({ southRam: results, eastRam: { settledX: +eastEdge.x.toFixed(2), settledZ: +eastEdge.z.toFixed(2), footY: +eastEdge.footY.toFixed(2) } }, null, 2));
