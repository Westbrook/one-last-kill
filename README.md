...
For a phone or tablet, enable **Settings → On-screen touch controls** before starting or from the pause menu. The option is off by default and saves locally. Touch the left movement area to place the stick's center under your thumb, then drag to move. Every new touch starts neutral. Without motion access, the original touch layout and gestures remain: swipe the right side to look, tap and release the large **FIRE** control to attack once, or drag it to aim without firing. **RUN** and **CROUCH** toggle on each tap. Tap **JUMP**, **USE** (pick up weapons or ammunition), **RELOAD**, **MELEE**, **DROP**, or **PAUSE** for those actions. Pausing clears held controls and toggles.

Tap **MOTION** during play and allow motion access if prompted. Once permission is granted, the swipe area and its hint disappear, touch-based looking is disabled, and combat buttons use a compact two-row layout. **FIRE** becomes a dedicated trigger: press to attack, hold to repeat automatic weapons, and release to stop; dragging it cannot turn the camera. Hold the phone comfortably for the first reading, then turn or tilt it to aim. Hold **RECENTER** while adjusting your grip, then release to resume aiming from that position. Touch aiming stays disabled while calibrating, recentering and resuming. **SIGHTS** still controls firearm sights; it does not turn the camera.

Tap **MOTION** again to return to the original touch layout and gestures. Motion aiming requires a secure context (HTTPS, or localhost during development), browser support and usable orientation readings. Pending or denied permission keeps the original controls usable; if initial sensor readings never arrive, those controls return automatically. Switching modes cancels existing look/fire gestures so they cannot turn into a shot or carry camera movement into the new mode. Motion permission is requested only from the explicit button gesture; the selection lasts for the current page session and resets when touch controls are disabled. **Look sensitivity** adjusts both swipe and motion aiming; weapon sights reduce sensitivity as usual. Pausing, resuming and rotating the display recalibrate without turning the camera.

The adapter uses relative rotations from [`deviceorientation`](https://www.w3.org/TR/orientation-event/#deviceorientation), so it does not need compass heading or magnetometer permission. Quaternion differences account for portrait/landscape orientation and angle wrapping, then feed yaw and pitch through the existing look input. Devices without sensors still need the swipe fallback. Automated tests cover synthetic sensor data and permission/session handling; aiming feel, sensor noise and browser permission prompts still need validation on physical iOS and Android devices.
