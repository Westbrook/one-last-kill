...
For a phone or tablet, enable **Settings → On-screen touch controls** before starting or from the pause menu. The option is off by default and saves locally. Enabling it requests motion access immediately from that tap or keyboard action where the browser supports it. Accepting permission activates motion aiming automatically; a separate **MOTION** tap is not required. A saved touch preference does not prompt when the page loads: the next Start, briefing-continue or Resume action supplies the required gesture. Controller polling and programmatic engagement do not request permission.

Touch the left movement area to place the stick's center under your thumb, then drag to move. Every new touch starts neutral. In swipe mode, swipe the right side to look, tap and release the large **FIRE** control to attack once, or drag it to aim without firing. In motion mode, the swipe area and its hint disappear and **FIRE** keeps the same large circular target and position, becoming a dedicated trigger: press to attack, hold to repeat automatic weapons, and release to stop. Dragging it cannot turn the camera. **RUN** and **CROUCH** toggle on each tap. Tap **JUMP**, **USE** (pick up weapons or ammunition), **RELOAD**, **MELEE**, **DROP**, or **PAUSE** for those actions. Pausing clears held controls and toggles.

Hold the phone comfortably for the first reading, then pan left/right or tilt up/down to aim. Hold **RECENTER** while adjusting your grip, then release to resume from that position. Touch aiming stays disabled while calibrating, recentering and resuming. The adapter uses the screen's forward direction from relative [`deviceorientation`](https://www.w3.org/TR/orientation-event/#deviceorientation) readings, with gravity defining up for an upright hold. Rolling the phone sideways, including with display rotation locked, does not exchange pan and tilt. A nearly flat starting hold uses a calibrated screen reference until the phone is lifted upright. Near a straight-up or straight-down pointing direction, horizontal sensitivity eases off; leaving an ambiguous direction establishes a fresh reference without a camera jump or inverted controls. Compass heading and magnetometer permission are not needed.

Motion aiming requires a secure context (HTTPS, or localhost during development), browser support and usable orientation readings. Pending or denied permission keeps swipe controls usable; if initial readings never arrive, swipe controls return automatically. Denial, unavailable sensors and choosing swipe mode do not trigger repeated permission requests on resume. **MOTION** remains an optional switch back to swipe aiming or an intentional retry. These choices last for the page session; a reload can request again from a new start/resume gesture. Switching modes cancels existing look/fire gestures so they cannot become a shot or carry camera movement into the new mode. **Look sensitivity** adjusts both swipe and motion aiming; weapon sights reduce sensitivity as usual. Pausing, resuming and rotating the display recalibrate without turning the camera. Automated tests cover synthetic sensor data and permission/session handling; physical iOS and Android aiming, sensor noise and permission dialogs remain untested.

**SIGHTS** is the firearm aim toggle: it brings the weapon to its sights, narrows the field of view, and tightens shot spread. It appears only when carrying a firearm; motion or swipe aiming still turns the camera. **RAGE** appears only while the player is eligible to enter rage. Both controls clear their touch state when they become unavailable.
...
**Automatic** graphics targets 60 fps using recent frame intervals, measured CPU work and optional asynchronous GPU timing. Long stalls remain in the diagnostics and delay quality recovery. Sustained pressure reduces quality; recovery requires a longer stable period without nearby live contacts. CPU pressure can reduce presentation work while keeping resolution, while GPU or otherwise unexplained pressure can lower render scale within 0.70–1.40×, capped by device pixel ratio. Reduced presentation tiers emit fewer cosmetic particles, bypass contact shading, and refresh shadows at nominal 30 or 20 Hz; a changed shadow projection refreshes immediately. Essential shot feedback, combat randomness and simulation rules stay consistent across tiers.

**High** and **Performance** remain fixed presets while still collecting timing diagnostics. High keeps full cosmetic density, contact shading and shadow updates each rendered frame. Performance uses a scale capped at 0.85×, reduced cosmetic density, and no contact shading or shadows. Automatic also bypasses contact shading below 1.0× effective scale. The player's hands and weapon retain their separate clear rendering pass. Light, particle, corpse and navigation budgets remain bounded.

Runtime diagnostics retain recent p95, p99 and maximum frame times, CPU/simulation/render timings, available GPU timings, and recent plus session stall counts. Timing samples use fixed buffers; sorting and diagnostic result objects are created only when inspected. Input and HUD sampling reuse output objects, motion calculations use scalar storage, and cosmetic quality changes keep effect pools resident. These changes reduce allocation pressure, but cannot control the browser's garbage collector or guarantee stall-free play. A long frame alone does not identify a GC pause.
...
The held pistol now has a Blender-authored model with an editable source, repeatable GLB export, and procedural loading fallback. The [Blender pistol pilot](docs/blender-pistol-pilot.md) records the modest visual refinement, preserved grip and sights, final captures, and geometry/performance checks.

The [Blender model rollout](docs/blender-model-rollout.md) extends prepared assets to the remaining weapons, character surfaces, hands, vehicles, supplies, furniture, and selected world dressing. It records delivery budgets, startup/fallback behavior, editable sources and rebuild commands, remaining procedural structure, 43 paired gameplay comparisons, and measured performance.

The [character and hand fidelity pass](docs/blender-character-hand-fidelity.md) records the accepted gunman remodel and eight hand shapes, editable sculpt/bake workflow, 29 paired views, and 17 timing reports against that rollout.
