// Test wrap walkway traversal at different z positions to see if there's any
// path through, and confirm the fire collider extrusion.
...
const SHIM = '\n;try{window.__G={Player,Colliders,Triggers,ZONE_WAVE_CONFIG,HealPickups,ZONE_HIDE_ON_ENTER,isBlocked,resolveSphereAABB,THREE};}catch(e){console.error(e);}\n';
...
const proc = spawn(CHROME, ['--remote-debugging-port=' + PORT, '--window-position=2000,2000', '--user-data-dir=/tmp/chrome-diag-wrap2', '--no-first-run', URL], { stdio: ['ignore', 'pipe', 'pipe'] });
...
// Sweep at multiple z positions across the south face of the apartment.
async function dropAt(x, z) {
  return await evalExpr(`(() => {
    const G = window.__G, Player = G.Player, Colliders = G.Colliders, THREE = G.THREE, R = G.resolveSphereAABB;
    Player.pos.set(${x}, 4.5 + Player._eyeH, ${z}); Player.vel.set(0,0,0);
    const dt = 1/60;
    for (let f = 0; f < 60; f++) {
      Player.vel.y -= 22 * dt; Player.pos.addScaledVector(Player.vel, dt);
    ...
    return { x: Player.pos.x, z: Player.pos.z, footY: Player.pos.y - Player._eyeH };
  })()`);
}

const grid = [];
for (let x = -10; x <= -3; x += 1) {
  for (let z = 0.1; z <= 1.25; z += 0.2) {
    const r = await dropAt(x, z);
    grid.push({ x, z: +z.toFixed(2), footY: +r.footY.toFixed(2), settledX: +r.x.toFixed(2), settledZ: +r.z.toFixed(2) });
  }
}

// Also: enumerate fire colliders extruding past z=0.
const fireColliders = await evalExpr(`(() => {
  const list = window.__G.Colliders.list;
  return list.map((b, i) => ({ i, min: b.min.toArray(), max: b.max.toArray() }))
    .filter(b => b.min[2] < 0 && b.max[2] > 0 && b.min[1] >= 3.5 && b.min[1] <= 5 && b.min[0] > -16 && b.max[0] < 12);
})()`);

writeFileSync('/tmp/verify-task6/diag-wrap2.json', JSON.stringify({ grid, fireColliders }, null, 2));
console.log(JSON.stringify({ grid, fireColliders }, null, 2));
